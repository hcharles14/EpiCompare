#global variables
H3K4me1_rpkm_path='coordinates/H3K4me1_rpkm_coord'
...
createLink <- function(chr,start,end) {
    hub_name=read.table('www/write_to_disk/hub_name_file',header=F) #read from hub name file to get the hub name
    sprintf('<a href="//epigenomegateway.wustl.edu/browser/?genome=hg19&datahub_jsonfile=http://epigenome.wustl.edu/epicompare/datahubs/%s&coordinate=%s:%i-%i"
   target="_blank" class="btn btn-primary">Link</a>',hub_name[[1]],chr,start,end)
}


#enrichment analysis 
cal_enrichment<-function(target_file,foredata,backdata){
    ...
    for (x in foredata){
        if (is.element(x, sample_list127)){
            ...
            if (is.element(x, sample_list98)){
                ...
                f_path=paste('H3K27ac_peak_sort/',x,'-H3K27ac.imputed.narrowPeak.bed.nPk_sort',sep='')
            }
...
#process user data
process_user_data=function(coord_file){
    #sort
    cmd=paste('sort -k1,1 -k2,2n  www/write_to_disk/userdata.txt > www/write_to_disk/userdata_sort.txt')
    system(cmd)
    cmd=paste('www/bedtools','intersect','-a', coord_file, '-b', 'www/write_to_disk/userdata_sort.txt','-sorted', '-c', '>www/write_to_disk/user_processed.txt')
    system(cmd)
}
...
generate_hub_file=function(features,samples,hub_name){
    #all features
    ...
    filename=paste('www/write_to_disk/',hub_name,sep='')
    cat(hub_name,file='www/write_to_disk/hub_name_file') #save the hub name to a file
    cat(exp,file=filename)
}
